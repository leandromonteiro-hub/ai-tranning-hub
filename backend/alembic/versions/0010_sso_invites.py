...
def upgrade() -> None:
    add_column_if_missing("athletes", sa.Column("google_sub", sa.String(length=64), nullable=True))
    create_index_if_missing("ix_athletes_google_sub", "athletes", ["google_sub"], unique=True)
    ...
    create_index_if_missing("ix_invite_codes_code", "invite_codes", ["code"], unique=True)


def downgrade() -> None:
    op.drop_index("ix_invite_codes_code", table_name="invite_codes")
    op.drop_table("invite_codes")
    drop_column_if_exists("athletes", "onboarding_completed_at")
    ...
    drop_column_if_exists("athletes", "google_sub")
